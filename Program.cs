    ...
    public class Program
    {
        ...
        private static async Task<List<CloudTask>> AddTasksAsync(BatchClient batchClient, string jobId, List<ResourceFile> inputFiles, string outputContainerSasUrl)
        {
            ...
            foreach (ResourceFile inputFile in inputFiles)
            {
                ...
                int currentTask = inputFiles.IndexOf(inputFile);

                if (currentTask == 0)
                {
                    task.EnvironmentSettings.Add(new EnvironmentSetting("TEST_ENV_SETTING_ONE", "ValueOne"));
                }
                else if (currentTask > 0)
                {
                    task.DependsOn = TaskDependencies.OnIds("Walts_Test_Task" + (currentTask - 1));

                    if (currentTask == 1)
                    {
                        task.EnvironmentSettings.Add(new EnvironmentSetting("TEST_ENV_SETTING_TWO", "ValueTwo"));
                    }
                    else if (currentTask == 2)
                    {
                        task.EnvironmentSettings.Add(new EnvironmentSetting("TEST_ENV_SETTING_THREE", "ValueThree"));
                    }
                }
